...
async function task () {
  ...
  //自动签到
  schedule.scheduleJob(BotConfig.pushTask.signTime, () => YunzaiApps.dailyNote.signTask(-1));
  //官方公告推送
...
//加载功能
export async function loadApp (dir) {
  let YunzaiApps = {};
  let command = [];
  for (let val of dir) {
...
async function firstLogin () {
  ...
  let v3Ready = false
  try {
    let obj = await import(`file://${process.cwd()}/lib/adapter/v3-dependencies-test.js`)
    v3Ready = obj.v3Ready === true
  } catch (e) {
    v3Ready = false
  }
  if (!v3Ready) {
    setTimeout(() => {
      common.relpyPrivate(BotConfig.masterQQ[0], '为了后续适配V3版本Yunzai，有新的依赖需要安装\n请输入【#npm安装依赖】或【#cnpm安装依赖】进行安装')
    }, 1500);
  } else {
    console.log('v3 is ready')
  }
}
