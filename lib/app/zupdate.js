...
export async function checkupdate (e) {
  ...
  let url = "https://api.github.com/repos/TimeRainStarSky/Yunzai-Bot-2"; //git地址
  let msg =
...
export async function setUpdatetime (e) {
  ...
  url = "https://api.github.com/repos/TimeRainStarSky/Yunzai-Bot-2/commits";
...
export async function updateLog (e) {
  ...
  let url = "https://api.github.com/repos/TimeRainStarSky/Yunzai-Bot-2/contents/resources/readme/更新日志.md?ref=master"; //git地址
  let response = await fetch(url); //调用接口获取数据
  let res = await response.json(); //结果json字符串转对象
...
export async function npmInstall (e, isCnpm = false) {
  if (!e.isMaster) {
    e.reply("您无权操作");
    return true;
  }
  let command = `${isCnpm === true ? 'cnpm' : 'npm'} install`;
  e.reply("正在尝试安装依赖，请稍候...");
  exec(command, function (error, stdout, stderr) {
    if (error) {
      e.reply("失败！\nError code: " + error.code + "\n" + error.stack + "\n\n，请尝试手工升级依赖");
    } else {
      if (/(added|removed|changed)\s*\d+\s*packages/.test(stdout.toString())) {
        e.reply("安装成功，请使用【#重启】命令重启..")
      } else {
        e.reply("当前已经安装全部的V3依赖")
      }
    }
  })
  return true
...
export async function cnpmInstall (e) {
  return await npmInstall(e, true)
}
