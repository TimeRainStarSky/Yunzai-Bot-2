...
export async function signTask (reSignCount = 0, replyMaster = false) {
  ...
  if (reSignCount === 0) {
    ...
    if (signNum >= 0) {
      ...
      let msg = [`签到开始，当前共计${needSign.totalCount}条CK记录`]
      let counts = []
      let addCount = (title, count = 0) => count > 0 && counts.push(`${title}:${count}`)
      addCount('需签到', signNum)
      addCount('已签', count.finish)
      addCount('未开签到', needSign.noSignCount)
      addCount('CK不全', needSign.errCkCount)
      addCount('CK已删或失效', count.invalid)
      msg.push(`[${counts.join(', ')}]`)
