...
export async function signTask() {
  ...
  let time = signNum * 15 + signNum * 0.1 + signNum * 0.02
  let finishTime = moment().add(time, 's').format('MM-DD HH:mm:ss')
  common.relpyPrivate(BotConfig.masterQQ[0], `签到ck：${signNum}个\n预计需要：${countTime(time)}\n完成时间：${finishTime}`);
  //获取需要签到的用户
  for (let [user_id, cookie] of Object.entries(NoteCookie)) {
    let qq = cookie.qq || user_id;
    if (!cookie.cookie.includes("cookie_token")) {
    ...
    common.relpyPrivate(BotConfig.masterQQ[0], `原神签到任务完成：${signNum}个\n成功：${sucNum}个\n已签：${finshNum}个\n失败：${failNum}个\n失效：${invalidNum}`)
