  ...
  async init () {
    ...
    if (successUids && successUids.length > 0) {
      for (let idx in successUids) {
        await SignCache.status(successUids[idx], SignStatus.finish)
      }
    }
  },
...
export async function sign (e) {
  ...
  if (res?.data?.risk_code == 375) {
    // 增加失败uid记录
    await SignCache.status(uid, 'code')
    e.reply(`签到失败：验证码失败`)
...
export async function signTask (reSignCount = 0, replyMaster = false) {
  ...
  if (reSignCount === 0) {
    ...
    signNote = await getNeedSignNote()
    let signNum = Object.keys(signNote).length;
    if (signNum === 0) {
      await replyMaster('所有用户均已经成功签到')
      return;
    }
    ...
    await replyMaster(`签到开始，本次预计签到${signNum}个记录\n需要${countTime(time)}，完成时间：${finishTime}`);
  } else {
    signNote = await getNeedSignNote()
  }
  //获取需要签到的用户
  for (let [user_id, cookie] of Object.entries(signNote)) {
    ...
    // 重签下判断是否在失败列表中
    if (reSignCount > 0 && !await SignCache.is(uid, 'fail')) {
      continue
    }
  ...
  msg.push(`签到错误：${count.invalid + count.error}个 [CK失效${count.invalid}, 接口错误${count.error}]`)
