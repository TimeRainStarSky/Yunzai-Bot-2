...
// 暂时自维护，后续可以开放至config
const signCfg = {
  signInterval: 1, // 签到间隔，单位秒
  rewardInterval: 0.5, // 签到过程中查询签到结果间隔，单位秒
  reSignInterval: 5, // 重签时间间隔，单位秒
  reSignLimit: 30, // 重试轮次上限
}
const SignStatus = {
  finish: 0, // 已签到
  success: 1, // 签到成功
...
export async function signTask (reSignCount = 0, replyMaster = false) {
  ...
  if (reSignCount === 0) {
    ...
    if (signNum > 0) {
      ...
      time = time * 1.6 + signCfg.reSignInterval * 5 // 预估重试需要额外消耗60%时间
      let finishTime = moment().add(time, 's').format('MM-DD HH:mm:ss')
      await replyMaster(`签到开始，共计${count.finish + count.invalid + signNum}记录\n本次预计签到${signNum}个，需要${countTime(time)}\n完成时间：${finishTime}`);
    }
  } else {
    signNote = await getNeedSignNote()
  }
  ...
  msg.push(`签到错误：${count.invalid + count.error + count.req}个 [CK失效${count.invalid}, 接口错误${count.error + count.req}]`)
