...
export async function signTask (reSignCount = 0, replyMaster = false) {
  // 判断默认关闭
  if (reSignCount === -1) {
    if (!BotConfig?.pushTask?.signTask) {
      Bot.logger.mark('自动签到已关闭，不进行自动签到')
      return;
    }
    reSignCount = 0
  }
  ...
  if (reSignCount === 0) {
    ...
    if (signNum > 0) {
      ...
      time = time * 2.1 // 预估重试需要额外消耗一倍时间
      let finishTime = moment().add(time, 's').format('MM-DD HH:mm:ss')
      let msg = [`签到开始，共计${needSign.totalCount}个CK，另有${count.invalid}个无效CK`]
  ...
  let needSign = await getNeedSignNote()
  let msg = [`原神签到任务完成：`]
  msg.push(`共计${needSign.totalCount}个CK，另有${count.invalid}个无效CK`)
  msg.push(getCountMsg({
  ...
  msg.push('')
  msg.push(`共计签到${count.sum}个记录，共签到${reSignCount}轮`)
  msg.push(`签到成功：${count.success + count.finish}个 ${getCountMsg({
    本次: count.success,
    已签: count.finish
  })}`)
  msg.push(`签到错误：${count.invalid + count.error + count.req + count.logout}个 ${getCountMsg({
    CK失效: count.invalid,
    CK登出: count.logout,
    接口错误: count.error,
    频次限制: count.req
