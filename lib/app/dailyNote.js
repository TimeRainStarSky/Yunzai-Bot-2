...
let invalidNum = 0; //失效
let errNum = 0;
const signFailKey = 'yunzai:sign-fail'
const inSignKey = 'yunzai:in-sign'

// 暂时自维护，后续可以开放至config
const signCfg = {
  signInterval: 2, // 签到间隔，单位秒
  rewardInterval: 0.5, // 签到过程中查询签到结果间隔，单位秒
  reSignInterval: 10, // 重签时间间隔，单位秒
  reSignLimit: 5, // 重试轮次上限
}
...
export async function sign (e) {
  ...
  if (signInfo.retcode != 0) {
    ...
    if (signInfo.message == "尚未登录" && cookie.includes("cookie_token")) {
      ...
      Bot.logger.mark(`米游社info接口错误[${user_id}]:${JSON.stringify(signInfo)}`);
    }
    return true;
  }
  await common.sleep((signCfg.rewardInterval || 1) * 1000)
  ...
  if (res?.data?.risk_code == 375) {
    // 增加失败uid记录
    await redis.sAdd(signFailKey, uid + '')
    e.reply(`签到失败：验证码失败`)
  ...
  if (res.retcode == 0 || res.retcode == -5003) {
    ...
    // 去除失败uid记录
    await redis.sRem(signFailKey, uid + '')
...
export async function signTask (reSignCount = 0, replyMaster = false) {
  ...
  let signNum = Object.keys(NoteCookie).length;
  if (reSignCount === 0) {
    // 首次签到重置计数
    if (await redis.get(inSignKey)) {
    ...
    invalidNum = 0; //失效
    errNum = 0; // 接口错误
    await redis.del(signFailKey);
    await redis.sAdd(signFailKey, 'in-sign')
    await redis.expire(signFailKey, common.getDayEnd())
    let time = signNum * (signCfg.signInterval + signCfg.rewardInterval)
    time = time * 1.6 // 预估重试需要额外消耗60%时间
    let finishTime = moment().add(time, 's').format('MM-DD HH:mm:ss')
    await replyMaster(`签到ck：${signNum}个\n预计需要：${countTime(time)}\n完成时间：${finishTime}`);
  }
  //获取需要签到的用户
  for (let [user_id, cookie] of Object.entries(NoteCookie)) {
    // 设置正在签到标记，时长2分钟
    await redis.set(inSignKey, 'in-sign', { EX: 2 * 60 })
    ...
    // 重签下判断是否在失败列表中
    if (reSignCount > 0 && !await redis.sIsMember(signFailKey, uid + '')) {
      continue
    }
    ...
    // 每个签到后等待指定之间
    await common.sleep((signCfg.signInterval || 2) * 1000);
  }
  let failNum = await redis.sCard(signFailKey) - 1 // 有一个默认占位，扣除1
  reSignCount++
  if (failNum > 0 && reSignCount <= signCfg.reSignLimit) {
    ...
    // 每轮签到后等待指定时间
    await common.sleep((signCfg.reSignInterval || 5) * 1000)
    await signTask(reSignCount, replyMaster)
    return
  }
  await replyMaster(`原神签到任务完成，共签到${reSignCount}轮：\nCK${signNum}个\n成功：${sucNum}个\n已签：${finshNum}个\n失败：${failNum}个\n失效：${invalidNum}个\n接口错误：${errNum}个`)
