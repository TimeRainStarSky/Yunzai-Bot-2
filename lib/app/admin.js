...
export async function delUser (e, { render, Models }) {
  let { MysUser } = Models;
  if (!(await e.checkAuth({ auth: "master" }))) {
    return true;
  }
  ...
  let delCount = 0;
  lodash.forEach(ret, (uid) => {
    lodash.forEach(NoteCookie, (v, k) => {
      if (v.uid * 1 === uid * 1) {
        delete NoteCookie[k];
        delCount++;
        return false
  ...
  await MysUser._delCache();
  e.reply(`共有${ret.length}个失效用户，已删除${delCount} 个`)
}
