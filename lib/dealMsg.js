...
import Runtime from './adapter/runtime.js'
...
async function dealMsg (e) {
  ...
  for (let val of e.message || []) {
    switch (val.type) {
      case "text":
        ...
        if (e.isPrivate && e.msg.indexOf("ltoken") == -1 && !e.msg.includes("_MHYUUID") && !e.isMaster && e
          .msg.indexOf("authkey_ver") == -
          1 && !/^[0-9]{9}$/.test(e.msg) && BotConfig.disablePrivate) {
          privateBool = true;
        }
        break;
      case "image":
        ...
        e.img.push(val.url);
        if (e.isPrivate && !e.isMaster && BotConfig.disablePrivate) {
          privateBool = true;
        }
        break;
      case "at":
      ...
      case "file":
        ...
        if (e.isPrivate && !e.isMaster && val.name.indexOf("txt") == -1 && BotConfig.disablePrivate) {
          privateBool = true;
        }
        break;
    }
  }
  if (privateBool && BotConfig.disablePrivate) {
    if (BotConfig.disableMsg) {
      e.reply(BotConfig.disableMsg)
    }
    return true;
  }
  //回复消息
  ...
  if (e.groupConfig.onlyReplyAt && e.isGroup && !e.isPoke) {
    ...
    if (alias && e.msg) {
      if (lodash.startsWith(e.msg.trim(), alias)) {
        ...
        e.atBot = true;
        e.msg = e.msg.replace(new RegExp("^\\s*" + alias + "\\s*"), "");
      }
    }
    if (!e.atBot) {
      return;
  ...
  for (let val of tmpCommand) {
    ...
    if (e.groupConfig.disable && e.groupConfig.disable.length > 0) {
      if (lodash.intersection(e.groupConfig.disable, [val.type, `${val.type}.${val.name}`, "all"]).length >
        0) {
        continue;
      }
    }
    ...
    if (e.isPoke && val.reg !== "#poke#") {
      continue;
    }

    if (new RegExp(val.reg).test(msg) || val.reg == "noCheck" || (e.isPoke && val.reg === "#poke#")) {
...
async function dealDecrease (e) {
  ...
  if (e.groupConfig.disable?.includes("decrease")) {
    return;
  }
  YunzaiApps.decrease.decrease(e).catch((err) => {
    Bot.logger.error(err);
  });
}
