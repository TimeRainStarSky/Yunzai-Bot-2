...
async function check() {
  if (fs.existsSync(configPath)) {
	...
	if (lodash.isEmpty(global.BotConfig.pushTask.isPushNote)) {
	  global.BotConfig.pushTask.isPushNote = 1;
	}
	if (lodash.isEmpty(global.BotConfig.pushTask.isPushNews)) {
	  global.BotConfig.pushTask.isPushNews = 1;
	}
	if (lodash.isEmpty(global.BotConfig.DisablePrivate)) {
